...
def get_mask_card_number(number_card: Optional[str] = None) -> str:
    """
    ...
    if type(number_card) is not str:
        masks_logger.error("Номер карты может быть только строчным значением!")
        raise TypeError("Номер карты может быть только строчным значением!")
