...
def get_mask_card_number(number_card: Optional[int] = None) -> str:
    ...
    if type(number_card) is not int:
        return "Номер карты может быть только числовым значением!"
    if number_card < 0:
        return "Номер карты не может быть отрицальным!"
    ...
    if len(mask_number_card_str) == 16:
        ...
        return "Номер карты состоит не из 16 цифр!"
...
def get_mask_account(number_account: Optional[int] = None) -> str:
    ...
    if type(number_account) is not int:
        return "Номер счёта может быть только числовым значением!"
    if number_account < 0:
        return "Номер счёта не может быть отрицальным!"
    mask_number_account_str = str(number_account)
    if len(mask_number_account_str) >= 6:
        return f"**{mask_number_account_str[-4:]}"
    else:
        return "Номер счёта не может быть меньше 6 цифт!"
